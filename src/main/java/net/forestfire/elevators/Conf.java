...
public class Conf {
	...
	public static String TITLE, CALL, ERROR, L_ST, L_END, NODOOR, MSG_GOTO_ST, MSG_GOTO_END, MSG_CALL, MSG_NOT_FOUND,
			MSG_PERM, MSG_PERM_END, NOMV, M_ATLV, ATLV, C_UP, UP, C_DOWN, DOWN;
	...
	public static final String MSG_NEW_CONF = "§e[Elevators] §bCould not load config. Creating new config file...",
			...
			MSG_DEL_END = " §esaved elevators were deleted because they were invalid!",
	...
	static void saveConf(boolean f) {
		...
		if (f)
			...
			SVTMR = plugin.setTimeout(Conf::doSaveConf, 200);
	}
	...
	private static void doSaveConf() {
		...
		for (int i = 0, l = BLOCKS.length; i < l; i++)
			bList.set(BLOCKS.get(i), BL_SPEED.get(i));
		...
		for (String k : elevators.keySet())
			eList.set(k, elevators.get(k).toSaveData()); // Gen Compressed Elev Data

		// Append New Data And Save File:
		data = data.substring(0, data.lastIndexOf("elevators:")) + eConf.saveToString();
		try {
			Writer w = new BufferedWriter(new OutputStreamWriter(Files.newOutputStream(f.toPath()), StandardCharsets.UTF_8));
			w.write(data);
			w.close();
	...
	static Object loadConf() {
		try {
			...
			if (bList != null)
				for (String k : bList.getKeys(false)) { // Remove non-sold blocks:
					if (getMat(k).isSolid()) {
						BLOCKS.add(k);
						BL_SPEED.add(bList.getInt(k));
					}
				}

			if (BLOCKS.length < 1) { // Load Default Block Settings:
				BLOCKS = new ChuList<>("IRON_BLOCK", "GOLD_BLOCK", "EMERALD_BLOCK", "DIAMOND_BLOCK", "LEGACY_STAINED_GLASS",
						"GLASS");
			...
			elevators.clear();
			ConfigurationSection eList = conf.getConfigurationSection("elevators");
			int eCnt = 0;
			if (eList != null)
				for (String k : eList.getKeys(false)) {
					Elevator e = Elevator.fromSaveData(eList.getStringList(k));
					if (e != null)
						elevators.put(k, e);
					else {
						eCnt++;
						err("loadConfig", "fromSaveData returned null at ID " + k);
					}
				}
	...
	private static Material getMat(String m) throws Exception {
		try {
			...
			throw new Exception("No such material " + m);
		}
	}
	...
	static void setDoor(Block b, boolean on) {
		if (b.getBlockData() instanceof Openable) {
			...
			if (d instanceof Door && ((Door) d).getHalf() != Bisected.Half.BOTTOM)
				return;
			d.setOpen(on);
			b.setBlockData(d);
			playDoorSound(b, on);
	...
	private static void playDoorSound(Block b, boolean on) {
		...
		Openable d = (Openable) b.getBlockData();
		Sound s;
		if (d instanceof Gate)
			s = on ? Sound.BLOCK_FENCE_GATE_OPEN : Sound.BLOCK_FENCE_GATE_CLOSE;
		else if (d instanceof TrapDoor) {
			if (b.getType() == Material.IRON_TRAPDOOR)
				s = on ? Sound.BLOCK_IRON_TRAPDOOR_OPEN : Sound.BLOCK_IRON_TRAPDOOR_CLOSE;
			else
				s = on ? Sound.BLOCK_WOODEN_TRAPDOOR_OPEN : Sound.BLOCK_WOODEN_TRAPDOOR_CLOSE;
		} else if (b.getType() == Material.IRON_DOOR)
			s = on ? Sound.BLOCK_IRON_DOOR_OPEN : Sound.BLOCK_IRON_DOOR_CLOSE;
		else
			s = on ? Sound.BLOCK_WOODEN_DOOR_OPEN : Sound.BLOCK_WOODEN_DOOR_CLOSE;
	...
	static void line(Block b, int i, String str) {
		...
		s.line(i, sc(str == null ? "" : str));
		s.update();
	}

	static String line(Block b, int i) {
		return cs(((Sign) b.getState()).line(i));
	}
	...
	static void addSignBlock(Block s) {
		setDoorBlock(Conf.getSignBlock(s), true);
	}

	static void setDoorBlock(Block b, boolean on) {
		Material m = b.getType();
		if (on ? !m.isSolid() : m == Conf.DOOR_SET)
	...
	static boolean isElevSign(Block b, ConfData ref, Player p) {
		if (!hasPerm(p, Main.PERM_USE, true))
			return false;
		if (b.getBlockData() instanceof WallSign && TITLE.equals(line(b, 0))) {
			ref.data = Elevator.fromElevSign(b);
			if (ref.data == null)
				p.sendMessage(MSG_NOT_FOUND);
	...
	static boolean isCallSign(Block b, ConfData ref, Player p) {
		if (!hasPerm(p, Main.PERM_USE, true))
			return false;
		if (b.getBlockData() instanceof WallSign && CALL.equals(line(b, 0))) {
			ref.data = Elevator.fromCallSign(b);
			if (ref.data == null)
				p.sendMessage(MSG_NOT_FOUND);
			return (ref.data != null);
		}
	...
	static boolean isElevPlayer(Player p, ConfData ref) {
		if (!hasPerm(p, Main.PERM_USE, false))
			return false;
		ref.data = Elevator.fromEntity(p);
		return (ref.data != null);
	}
	...
	static boolean hasPerm(Player p, String perm, boolean m) {
		...
		if (m && !h)
			p.sendMessage(MSG_PERM + perm + MSG_PERM_END);
		return h;
	}
	...
	static String cs(Component c) {
		return LegacyComponentSerializer.legacyAmpersand().serialize(c);
	}

	static Component sc(String s) {
		return LegacyComponentSerializer.legacyAmpersand().deserialize(s);
	}

	static String c(String s) {
		return LegacyComponentSerializer.legacySection().serialize(sc(s));
	}
	...
	static void dbg(String str) {
		if (DEBUG) {
			...
			for (Player p : pl)
				if (hasPerm(p, Main.PERM_RELOAD, false))
					p.sendMessage(msg);
		}
	}
