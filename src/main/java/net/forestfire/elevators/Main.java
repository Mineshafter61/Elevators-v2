	...
	public void onSignChange(SignChangeEvent e) {
		...
		synchronized (Conf.API_SYNC) {
			if (sign.getBlockData() instanceof WallSign) {
				if (Conf.cs(e.line(0)).equalsIgnoreCase("[call]") && Conf.hasPerm(e.getPlayer(), PERM_CREATE, true)) { // Call
																																																								// Sign:
					...
					if (elev != null) { // Existing Elevator:
						...
						if (col != -1) { // Existing Column:
							...
							for (int k = 0, m = elev.sGroups.length; k < m; k++)
								if (k != col) { // Rebuild other cols to match
					...
					for (int i = 0; i < sList.length; i++) {
						if (i > 0 && (sList.get(i).getY() - sList.get(i - 1).getY()) < 3) { // Signs too close!
							...
							for (ChuList<Block> g : elev.sGroups)
								Conf.lines(g.remove(i), new String[] { Conf.ERROR, "", "", "" });
							i--;
						}
					}
	...
	public void onPlayerInteract(PlayerInteractEvent e) {
		synchronized (Conf.API_SYNC) {
			...
			if (act == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock().getBlockData() instanceof WallSign) {
				if (Conf.isElevSign(e.getClickedBlock(), ref, e.getPlayer()) && !((Elevator) ref.data).floor.moving) { // Select
																																																								// Floor:
					...
					e.setCancelled(true);
				} else if (Conf.isCallSign(e.getClickedBlock(), ref, e.getPlayer()) && !((CSData) ref.data).elev.floor.moving) { // Call
																																																													// Sign
																																																													// Click:
