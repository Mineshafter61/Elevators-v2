...
public class Main extends JavaPlugin implements Listener 
{
    ...
    @Override public boolean onCommand(CommandSender s, Command c, String l, String[] a) 
    {
        if (c.getName().equalsIgnoreCase("elev")) 
        {
            if (a.length == 1 && a[0].equals("list")) 
            {
                ...
                {
                    Floor f = e.floor;
                    Block b = e.sGroups.get(0).get(0);
                    s.sendMessage("Elevator in §d" + f.world.getName()
                            + " §rat §b[" + b.getX() + "," + b.getZ() + "]§r; Size: §b" + (f.xMax - f.xMin) + "x"
                            + (f.zMax - f.zMin));
                }
            }
            else if (a.length > 0 && a[0].equals("reset")) 
            {
                if (LAST_ELEV != null) 
                {
                    ...
                    s.sendMessage("§cNo last elevator.");
            }
            else if (a.length == 1 && a[0].equals("reload")) 
            {
                ...
                s.sendMessage("§cUsage: /elev <list|reload|reset>");
            return true;
        }
        return false;
    }
    ...
    public void setInterval(BukkitRunnable f, long ms) 
    {
        ms /= 50;
        f.runTaskTimer(this, ms, ms);
    }

    // ------------------- Elevator Create Sign Events -------------------

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onSignChange(SignChangeEvent e) 
    {
        ...
        synchronized (Conf.API_SYNC) 
        {
            ...
            {
                if (Conf.cs(e.line(0)).equalsIgnoreCase("[call]") && Conf.hasPerm(e.getPlayer(), PERM_CREATE, true)) 
                { // Call
                  // Sign:
                    e.line(0, Conf.sc("> ERROR <"));
                    e.line(1, Conf.sc(""));
                    e.line(2, Conf.sc(""));

                    // Is sign in elevator?
                    CSData ret = Elevator.fromCallSign(sign);
                    if (ret == null) 
                    {
                    ...
                    e.line(0, Conf.sc(Conf.CALL));
                    e.line(3, Conf.sc(elev.csData.get(csNum)));

                }
                else if (Conf.cs(e.line(0)).equalsIgnoreCase("[elevator]")
                        && Conf.hasPerm(e.getPlayer(), PERM_CREATE, true)) 
                { // Elevator Sign:
                    e.line(0, Conf.sc("> ERROR <"));
                    e.line(1, Conf.sc(""));
                    ...
                    { // Existing Elevator:
                        ...
                        int sX = sign.getX(), sY = sign.getY(), sZ = sign.getZ();
                        for (int i = 0, l = elev.sGroups.length; i < l; i++) 
                        { // Check for col XZ match:
                            Block ds = elev.sGroups.get(i).get(0);
                            if (ds.getX() == sX && ds.getZ() == sZ) 
                            {
                                col = i;
                                break;
                            }
                        ...
                        { // New Column:
                            ...
                            Material t = sign.getType();
                            sign.setType(Conf.AIR);
                            ChuList<Block> sRef = elev.sGroups.get(0);
                            int sl = sRef.length;
                            sList = new ChuList<>(sl);
                            for (int i = 0; i < sl; i++) 
                            { // Rebuild to match other cols
                                Block b = w.getBlockAt(sX, sRef.get(i).getY(), sZ);
                                b.setType(t);
                                b.setBlockData(d);
                                Conf.addSignBlock(b);
                                Conf.lines(b, Conf.lines(sRef.get(i)));
                                sList.add(b);
                            }
                        }
                    }
                    ...
                    { // New elevator:
                        Floor f = Floor.getFloor(sign, null);
                        if (f == null) 
                        {
                            e.line(0, Conf.sc(Conf.ERROR));
                    ...
                    if (col == -1)
                        elev.sGroups.add(sList);
                    else
                        elev.sGroups.set(col, sList);
                    if (elev.noDoor)
                        elev.sGroups.get(0).forEach((s) -> Conf.line(s, 2, "")); // NoDoor
                    ...
                    for (int i = 0; i < sList.length; i++) 
                    {
                        ...
                        { // Signs too close!
                            if (sList.get(i).getY() == sign.getY())
                                cErr = true;
                            for (ChuList<Block> g : elev.sGroups)
                                Conf.lines(g.remove(i), new String[] 
                                        { Conf.ERROR, "", "", "" });
                            i--;
                        }
                    }
                    ...
                    if (elev.noDoor || sl.equalsIgnoreCase("[nodoor]")) 
                    { // Enable NoDoor Mode:
                        elev.noDoor = true;
                        Block f = elev.sGroups.get(0).get(0);
                        if (f.equals(sign))
                            e.line(2, Conf.sc(Conf.NODOOR));
                        else 
                        {
                            Conf.line(f, 2, Conf.NODOOR);
                            e.line(2, Conf.sc(""));
                        }
                    }
                    else
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onBlockPhysics(BlockPhysicsEvent e) 
    {
        synchronized (Conf.API_SYNC) 
        {
            Block b = e.getBlock();
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onBlockBreak(BlockBreakEvent e) 
    {
        synchronized (Conf.API_SYNC) 
        {
            Block b = e.getBlock();
            if (b.getBlockData() instanceof WallSign) 
            {
                if (Conf.CALL.equals(Conf.line(b, 0)))
                    onDestroyCallSign(e, e.getPlayer(), b); // Call Sign
                else if (Conf.TITLE.equals(Conf.line(b, 0)))
                    onDestroyElevSign(e, e.getPlayer(), b); // Elevator Sign
            }
            else if (Elevator.fromElevBlock(b) != null) 
            { // Block Door:
                e.setCancelled(true);
                b.setType(Conf.AIR);
                Conf.dbg("PreventBDDrop"); // Prevent Survival dupe exploit!
            }
        }
    }
    ...
    private void onDestroyElevSign(Cancellable e, Player p, Block b) 
    {
        ...
        // Build sign list:
        ChuList<Block> sList = Elevator.rebuildSignList(b);
        elev.resetElevator();

        // Find sGroups index:
        int ind = -1;
        for (int i = 0, l = elev.sGroups.length; i < l; i++)
            if (b.getX() == elev.sGroups.get(i).get(0).getX()
                    && b.getZ() == elev.sGroups.get(i).get(0).getZ()) 
            {
                ind = i;
                break;
            }
        ...
        { // Only One Column Left:
            ...
            for (int i = 0, l = sList.length; i < l; i++)
                if (i != sInd)
                    f.addFloor(sList.get(i).getY() - 2, false, true); // Add floors.
            ChuList<Block> cs = elev.csGroups.get(sInd);
            if (cs != null)
                for (int h = 0, d = cs.length; h < d; h++)
                    cs.get(h).setType(Conf.AIR); // Delete call signs on level.
            if (sList.length <= 1) 
            { // Delete elevator. This meeting... never happened.
                elev.selfDestruct();
        ...
        if (pBl.isEmpty())
            pBl.setType(Material.DIRT); // Add block below player.
        if (elev.floor != null) 
        {
            elev.updateSignNames(null);
            elev.rebuildCallSignList(null);
        }
        Conf.saveConf(); // Save Changes To Config.
    }
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onPistonExtend(BlockPistonExtendEvent e) 
    {
        synchronized (Conf.API_SYNC) 
        {
            for (Block b : e.getBlocks())
                if (Elevator.fromElevBlock(b) != null) 
                {
                    e.setCancelled(true);
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onPistonRetract(BlockPistonRetractEvent e) 
    {
        synchronized (Conf.API_SYNC) 
        {
            for (Block b : e.getBlocks())
    ...
    @EventHandler public void onPlayerInteract(PlayerInteractEvent e)         
    {
        synchronized (Conf.API_SYNC) 
        {
            ...
            {
                if (Conf.isElevSign(e.getClickedBlock(), ref, e.getPlayer()) && !((Elevator) ref.data).floor.moving) 
                { // Select
                  // Floor:
                    Elevator elev = ((Elevator) ref.data);
                    LAST_ELEV = elev;
                    FList fl = elev.getFloors();
                    int sn = fl.sn;
                    ...
                    else if (++sn >= fl.fl.length)
                        sn = 0;
                    elev.updateFloorName(fl.fl.get(sn));

                    e.setCancelled(true);
                }
                else if (Conf.isCallSign(e.getClickedBlock(), ref, e.getPlayer()) && !((CSData) ref.data).elev.floor.moving) 
                { // Call
                  // Sign
                  // Click:
                    Elevator elev = ((CSData) ref.data).elev;
                    int from = elev.getFloor(), to = ((CSData) ref.data).index;
                    LAST_ELEV = elev;
                    ...
                    if (from != to) 
                    {
                        ...
                        elev.doorTimer(to); // Re-open doors if already on level.

                    e.setCancelled(true);
                }
            }
            else if ((act == Action.RIGHT_CLICK_AIR
                        || act == Action.RIGHT_CLICK_BLOCK && (e.getItem() == null || e.getPlayer().isSneaking()))
                    && Conf.isElevPlayer(e.getPlayer(), ref) && !((Elevator) ref.data).floor.moving) 
            { // Goto Floor:
                Elevator elev = ((Elevator) ref.data);
                LAST_ELEV = elev;
