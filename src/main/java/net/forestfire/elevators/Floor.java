...
public class Floor
{
    ...
    public static Floor getFloor(Block b, Elevator parent)
    {
        ...
        BlockFace f = ((WallSign) b.getBlockData()).getFacing();
        if (f != BlockFace.WEST)
            while (xP <= Conf.RADIUS_MAX)
            {
                if (world.getBlockAt(bX + xP, h, bZ).getType() != fType)
                    break;
                xP++;
            }
        if (f != BlockFace.EAST)
            while (xN <= Conf.RADIUS_MAX)
            {
                if (world.getBlockAt(bX - xN, h, bZ).getType() != fType)
                    break;
                xN++;
            }
        if (f != BlockFace.NORTH)
            while (zP <= Conf.RADIUS_MAX)
            {
                if (world.getBlockAt(bX, h, bZ + zP).getType() != fType)
                    break;
                zP++;
            }
        if (f != BlockFace.SOUTH)
            while (zN <= Conf.RADIUS_MAX)
            {
                if (world.getBlockAt(bX, h, bZ - zN).getType() != fType)
                    break;
                zN++;
            }
    ...
    public int addFloor(double h, boolean isMoving, boolean dontDelete, Integer forceID)
    {
        ...
        if (isMoving)
        { // Create FallingBlock Floor:
            ChuList<FallingBlock> bl = new ChuList<>((xMax - xMin + 1) * (zMax - zMin + 1));
            moving = true;
            for (int x = xMin;
                    x <= xMax;
                    x++)
            ...
            if (forceID != null)
                ...
                ind = Conf.findFirstEmpty(Conf.movingFloors);
            Conf.movingFloors.set(ind, bl);
            return ind;
        }
        else
        { // Create Solid Floor:
            for (int x = xMin;
                ...
                for (int z = zMin;
                        z <= zMax;
                        z++)
                    world.getBlockAt(x, (int) h, z).setType(fType);
        }
        return 0;
    }
    ...
    public void removeFallingBlocks()
    {
        ...
        for (Entity e : world.getEntitiesByClass(FallingBlock.class))
        {
            ...
            if ((l.getX() >= xMin - .5 && l.getX() <= xMax + .5) &&
                    (l.getY() >= yMin - .5 && l.getY() <= yMax + .5) && (l.getZ() >= zMin - .5 && l.getZ() <= zMax + .5))
                e.remove();
        }
    }
}
